...
static void sync_gauge_field(suNg_field *gf) {
  ...
  for (i = 0; i < gd->ncopies_gauge; ++i) {
    ...
    memcpy(((gf->ptr) + 4 * gd->copy_to[i]), ((gf->ptr) + 4 * gd->copy_from[i]),
           4 * (gd->copy_len[i]) * sizeof(*(gf->ptr)));
    /*
...
static void sync_spinor_field(spinor_field *p) {
  ...
  for (i = 0; i < gd->ncopies_spinor; ++i) {
    memcpy((p->ptr + gd->copy_to[i] - gd->master_shift),
           (p->ptr + gd->copy_from[i] - gd->master_shift),
           (gd->copy_len[i]) * sizeof(*(p->ptr)));
    /*
       for(j=0; j<gd->copy_len[i]; j++) {
       x=gd->copy_from[i]+j;
       y=gd->copy_to[i]+j;
...
/* This variable contains the information of the current status of
 * communications Values: 0 => No communications pending
 *
 */
...
void complete_gf_sendrecv(suNg_field *gf) {
  ...
  if (gf_control > 0) {
    ...
    lprintf("MPI TIMING", 0,
            "complete_gf_sendrecv" __FILE__ " %ld sec %ld usec\n",
...
void start_gf_sendrecv(suNg_field *gf) {
...
#ifdef MPI_TIMING
  error(gf_control > 0, 1, "start_gf_sendrecv " __FILE__,
        "Multiple send without receive");
  ...
  for (i = 0; i < (gd->nbuffers_gauge); ++i) {
    /* send ith buffer */
    mpiret =
        MPI_Isend((double *)((gf->ptr) + 4 * gd->sbuf_start[i]), /* buffer */
                  (gd->sbuf_len[i]) * sizeof(suNg) / sizeof(double) *
                      4,                 /* lenght in units of doubles */
    ...
    /* receive ith buffer */
    mpiret = MPI_Irecv(
        (double *)((gf->ptr) + 4 * gd->rbuf_start[i]), /* buffer */
        (gd->rbuf_len[i]) * sizeof(suNg) / sizeof(double) *
            4,                     /* lenght in units of doubles */
        MPI_DOUBLE,                /* basic datatype */
...
void complete_sf_sendrecv(spinor_field *sf) {
  ...
  if (sf_control > 0) {
    ...
    lprintf("MPI TIMING", 0,
            "complete_sf_sendrecv" __FILE__ " %ld sec %ld usec\n",
...
void start_sf_sendrecv(spinor_field *sf) {
...
#ifdef MPI_TIMING
  error(sf_control > 0, 1, "start_sf_sendrecv " __FILE__,
        "Multiple send without receive");
  ...
  for (i = 0; i < (gd->nbuffers_spinor); ++i) {
    /* send ith buffer */
    mpiret = MPI_Isend(
        (double *)((sf->ptr) + (gd->sbuf_start[i]) -
                   (gd->master_shift)), /* buffer */
        (gd->sbuf_len[i]) * (sizeof(suNf_spinor) /
                             sizeof(double)), /* lenght in units of doubles */
        MPI_DOUBLE,                           /* basic datatype */
        gd->sbuf_to_proc[i],                  /* cid of destination */
        i,                                    /* tag of communication */
        cart_comm,             /* use the cartesian communicator */
    ...
    /* receive ith buffer */
    mpiret = MPI_Irecv(
        (double *)((sf->ptr) + (gd->rbuf_start[i]) -
                   (gd->master_shift)), /* buffer */
        (gd->rbuf_len[i]) * (sizeof(suNf_spinor) /
                             sizeof(double)), /* lenght in units of doubles */
        MPI_DOUBLE,                           /* basic datatype */
        gd->rbuf_from_proc[i],                /* cid of origin */
        i,                                    /* tag of communication */
